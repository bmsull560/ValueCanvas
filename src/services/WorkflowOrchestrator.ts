...
export class WorkflowOrchestrator {
  ...
  private async executeStageWithRetry(
    ...
    for (let attempt = 1; attempt <= retryConfig.max_attempts; attempt++) {
      ...
      try {
        ...
        this.recordCircuitBreakerFailure(circuitBreakerKey, executionId, stage.id);
  ...
  private recordCircuitBreakerFailure(key: string, executionId: string, stageId: string): void {
