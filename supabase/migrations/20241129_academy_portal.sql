...
-- Index for ordering
CREATE INDEX idx_academy_modules_pillar_order ON academy_modules(pillar, display_order);
...
CREATE INDEX idx_academy_lessons_order ON academy_lessons(module_id, display_order);
...
CREATE INDEX idx_academy_progress_status ON academy_progress(user_id, status);
...
-- Index
CREATE INDEX idx_academy_certifications_user ON academy_certifications(user_id);
...
CREATE INDEX idx_value_ledger_value ON value_ledger(value_realized DESC);
...
-- Indexes
CREATE INDEX idx_resource_artifacts_stage ON resource_artifacts(lifecycle_stage);
CREATE INDEX idx_resource_artifacts_type ON resource_artifacts(artifact_type);
CREATE INDEX idx_resource_artifacts_active ON resource_artifacts(deprecated) WHERE deprecated = FALSE;
...
-- Index
CREATE INDEX idx_contextual_triggers_enabled ON contextual_triggers(enabled) WHERE enabled = TRUE;
