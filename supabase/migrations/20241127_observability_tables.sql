...
CREATE TABLE IF NOT EXISTS llm_calls (
  ...
  INDEX idx_llm_calls_session (session_id),
  INDEX idx_llm_calls_agent (agent_id),
  INDEX idx_llm_calls_provider (provider),
  INDEX idx_llm_calls_created (created_at DESC),
  INDEX idx_llm_calls_cache_hit (cache_hit)
);
...
CREATE TABLE IF NOT EXISTS value_prediction_accuracy (
  ...
  -- Indexes
  INDEX idx_value_prediction_accuracy_type (prediction_type),
  INDEX idx_value_prediction_accuracy_created (created_at DESC),
  INDEX idx_value_prediction_accuracy_measurement (measurement_date DESC)
);
...
CREATE TABLE IF NOT EXISTS confidence_violations (
  ...
  -- Indexes
  INDEX idx_confidence_violations_agent (agent_type),
  INDEX idx_confidence_violations_type (violation_type),
  INDEX idx_confidence_violations_created (created_at DESC)
);
...
CREATE MATERIALIZED VIEW IF NOT EXISTS value_prediction_accuracy_metrics AS
SELECT
  prediction_type,
...
COMMENT ON MATERIALIZED VIEW agent_performance_metrics IS 'Aggregated agent performance metrics by hour';
COMMENT ON MATERIALIZED VIEW llm_performance_metrics IS 'Aggregated LLM performance metrics by hour';
COMMENT ON MATERIALIZED VIEW value_prediction_accuracy_metrics IS 'Aggregated value prediction accuracy metrics by day';
